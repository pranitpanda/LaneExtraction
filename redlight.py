...
class MyCVController:
    ...
    def run(self, cam_img):
        ...
        blur_mask = cv2.GaussianBlur(gray_mask, (5, 5), 0)
        ret, thresh = cv2.threshold(blur_mask, 70, 255, 0)
        threshcopy = signs_region_of_interest(thresh)
        cv2.imwrite('cropped.jpg',threshcopy)
        thresh2, contours, hierarchy = cv2.findContours(threshcopy, 1, cv2.CHAIN_APPROX_SIMPLE)
        contours.sort(key=cv2.contourArea, reverse=True)
